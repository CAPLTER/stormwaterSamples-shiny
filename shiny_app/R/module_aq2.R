        ...
        fluidPage(
          fluidRow(
            column(id = "leftPanel", 2,
              machineInputUI(ns("aq2Samples")) # ns(wrap call to inner mod)
              ), # close the left col
...
aq2 <- function(input, output, session, tab = NULL) {
  ...
  resultReactive <- reactive({
    ...
    aq2_results <- rawReactive() %>%
      filter(!(grepl("c c|cc|standard|tres|digested|control|om|can|apa|roos", sample_id, ignore.case = T)))
